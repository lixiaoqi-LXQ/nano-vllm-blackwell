...
class Attention(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        if k_cache.numel() and v_cache.numel():
            if self.use_fp8_kv:
                # Quantize K/V to FP8 before storing
    ...
    def _flashattn_forward(self, q, k, v, k_cache, v_cache, context):
        """Original flash_attn path for BF16/FP8 models."""
    ...
    def _flashinfer_forward(self, q, k_cache, v_cache, context):
        """FlashInfer path: native FP8 KV cache support."""
        self._ensure_flashinfer_wrappers()

        # Use the pre-allocated kv_cache tensor [num_blocks, 2, block_size, num_kv_heads, head_dim]
        ...
        if context.is_prefill:
            ...
            o = wrapper.run(q, kv_cache, k_scale=self.k_scale, v_scale=self.v_scale)
        else:
