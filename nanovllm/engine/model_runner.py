...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        print(f"GPU Memory (GB) - Total: {total / 2**30:.2f} GB, Used: {used / 2**30:.2f} GB, Peak: {peak / 2**30:.2f} GB, Current: {current / 2**30:.2f} GB")
        num_kv_heads = hf_config.num_key_value_heads // self.world_size
        head_dim = getattr(hf_config, "head_dim", hf_config.hidden_size // hf_config.num_attention_heads)
        kv_dtype = torch.float8_e4m3fn if self.use_fp8_kv else hf_config.dtype
        ...
        print(f"Allocating KV cache -  Total blocks: {num_blocks}, KV Cache size: {self.block_size*num_blocks}")
        if self.use_fp8_kv:
