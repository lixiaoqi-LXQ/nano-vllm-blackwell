...
def get_model_class(path: str, hf_config) -> Type[nn.Module]:
    """Detect model type and return appropriate model class.

    Checks for FP8 quantized weights by looking for _scale_inv suffixes
    in the safetensors files.

    Args:
        path: Path to directory containing model files
        hf_config: HuggingFace config to determine model architecture

    Returns:
        Model class (FP8 or BF16 variant)
    """
    has_fp8 = False
    ...
    # Import appropriate model class based on architecture and quantization
    model_type = getattr(hf_config, "model_type", "qwen3")

    if model_type == "qwen3" or model_type == "qwen2":
        if has_fp8:
            from nanovllm.models.qwen3_fp8 import Qwen3FP8ForCausalLM
            return Qwen3FP8ForCausalLM
        else:
